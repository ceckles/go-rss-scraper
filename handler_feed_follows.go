...
// handlerGetFeeds returns users feeds from the database
...
// handlerDeleteFeedFollow deletes a feed follow from the database
func (apiCfg *apiConfig) handlerDeleteFeedFollow(w http.ResponseWriter, r *http.Request, user database.User) {
	feedFollowIDStr := chi.URLParam(r, "feedFollowID")
	feedFollowID, err := uuid.Parse(feedFollowIDStr)
	if err != nil {
		respondWithError(w, 400, fmt.Sprintf("Error Couldn't parse feed follow id: %v", err))
		return
	}
	...
	if err != nil {
		respondWithError(w, 400, fmt.Sprintf("Error Couldn't delete feed follow: %v", err))
		return
	}
